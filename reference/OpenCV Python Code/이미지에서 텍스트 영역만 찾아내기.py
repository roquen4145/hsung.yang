...
def find_border_components(countours,ary):
	borders = []
	...
	while covered_sum < total:
		...
		for i , c in enumerate(c_info):
			...
			if new_f1 > f1 or (remaining_frac > 0.25 and new_area_frac < 0.15):
				print('%d %s -> %s / %s (%s) , %s -> %s / %s (%s), %s -> %s' %(i,covered_sum,new_sum,total,remaining_frac,
					crop_area(crop), crop_area(new_crop),area,new_area_frac,f1,new_f1))
				crop = new_crop
				covered_sum = new_sum
				del c_info[i]
				changed = True
...
def pad_crop(crop,contours,edges,border_contour,pad_px=15):
	...
	def crop_in_border(crop):
		...
		x2 = max(x2 + pad_px, bx2)
		y2 = max(y2 + pad_px, by2)
		return crop
...
def process_image(path,out_path):
	...
	edges = 255*(edges>0).astype(np.uin8)
